...
public class Supervisor extends AbstractActor {
    ...
    private final Stack<ArrayList<ActorRef>> mergeStack = new Stack<>();
    private int layer = 0;
    ...
    public List<ActorRef> instantiateFilterOperator(FilterNode node) {
        ...
        List<ActorRef> downstreamOperator = instantiateOperator(node.getNext().get(0));
    ...
    public List<ActorRef> instantiateMergeOperator(MergeNode node) {
        int layer = this.layer ++;
        ArrayList<ActorRef> result;
        if(mergeStack.peek().size() == 0) {
            result = mergeStack.pop();
            lastLayerSize = node.getNumOperators();
            List<ActorRef> downstreamOperator = instantiateOperator(node.getNext().get(0));
            for (int i = 0; i < node.getNumOperators(); i++) {
                ActorRef actorRef = getContext()
            ...
            mergeStack.push(result);
        }
        return mergeStack.peek();
    }
